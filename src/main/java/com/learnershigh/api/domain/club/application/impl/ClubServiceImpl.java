package com.learnershigh.api.domain.club.application.impl;

import com.learnershigh.api.domain.club.application.ClubService;
...
import com.learnershigh.api.global.util.FileUploadUtil;
import com.learnershigh.api.domain.club.dto.ClubCreateRequestDto;
import com.learnershigh.api.domain.club.dto.ClubUpdateRequestDto;
...
public class ClubServiceImpl implements ClubService {
	...
	public void enterClub(Long clubId, String password){
		...
		if(/*club.getMaximumUserCount() < "현재 클럽 인원수(JPA)" + 1 */false){
			...
			//fixme: 일단 테이블의 1번째 유저가 입장되도록 하드코딩함
	...
	public boolean delegateClubMasterPrivileges(Long clubId, String delegatedUsername) throws AuthenticationException{
		...
		if("API를 호출한 유저가 클럽관리자인가?".equals(club.getMaster()) || true){
			...
			// FIXME: 2022/06/29 master 부분 수정필요
			club.setMaster(memberRepository.findOneByUsername(delegatedUsername).get());
